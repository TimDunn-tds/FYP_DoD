...
static void raise_PC4(void) { mock_gpioc.IDR |= GPIO_PIN_4; }       // Raise PC4
static void lower_PC4(void) { mock_gpioc.IDR &= ~(GPIO_PIN_4); }    // Lower PC4
...
{
    TEST_ASSERT_MESSAGE(GPIO_PIN_4 == (GPIO_Init_Spy.Pin & GPIO_PIN_4), "Expected GPIO PC4 to be initialised");
...
TEST(Button, InterruptEnabled)
{
    ...
    button_init();
    TEST_ASSERT_MESSAGE(1 == NVIC_CheckEnableIRQspy(EXTI4_IRQn), "Expected EXTI4 to be enabled in the NVIC");
}

TEST(Button, InterruptCleared)
{
    __HAL_GPIO_EXTI_SET_IT(GPIO_PIN_4);
    EXTI4_IRQHandler();
    TEST_ASSERT_MESSAGE(0 == __HAL_GPIO_EXTI_GET_IT(GPIO_PIN_4), "HAL_GPIO_EXTI_IRQHandler() must be called and clear the Pin 4 intterupt flag in EXTI4_IRQHandler()");
}
...
TEST(Button, InterruptPressed)
{
    ...
    TEST_ASSERT_EQUAL_MESSAGE(1, testGet, "Expected event flag to be set in ISR. Ensure button_isr() is called from EXTI4_IRQHandler()");
}
...
{
    raise_PC4();        //Raise PC4 to test 'rising edge'
    button_isr();       //Sets BUTTON_SIGNAL event flag
    button_debounce();  //Waits for event flag reads GPIO pin and sets state.
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(1,  button_get_pressed(), "Expected PC4 rising edge to set _was_pressed flag");
    uint32_t testGet = osEventFlagsGet(_spyButtonEventID);
    TEST_ASSERT_EQUAL_MESSAGE(0, testGet, "Expected event flags to be cleared at end of switch-case");

    lower_PC4();        //Lower PC4 to test 'falling edge'
    button_isr();       //Sets BUTTON_SIGNAL event flag
    button_debounce();  //Waits for event flag reads GPIO pin and sets state.
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(1,  button_get_pressed(), "Expected PC4 falling edge to retain state of _was_pressed flag");
