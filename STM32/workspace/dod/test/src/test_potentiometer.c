...
TEST(Potentiometer, ClocksInPotInit)
{
    ...
    pot_init();
    TEST_ASSERT_MESSAGE(1 == __HAL_RCC_ADC1_IS_CLK_ENABLED(), "Expected ADC1 clock to be enabled in pot_init().");
    TEST_ASSERT_MESSAGE(1 == __HAL_RCC_GPIOA_IS_CLK_ENABLED(), "Expected GPIOA clock to be enabled in pot_init().");
}
...
TEST(Potentiometer, PinConfig)
{
    ...
    hadc1.Instance = ADC1;
    pot_init();
    TEST_ASSERT_MESSAGE(GPIO_PIN_0 == (GPIO_Init_Spy.Pin & GPIO_PIN_0), "Expected pot_init() to initialise PA0");
...
TEST_GROUP_RUNNER(Potentiometer)
{
    ...
    RUN_TEST_CASE(Potentiometer, ClocksInPotInit);
