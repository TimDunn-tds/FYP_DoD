...
TEST(RGB_LED, ClocksInRGBInit)
{
    ...
    led_rgb_init();
    TEST_ASSERT_MESSAGE(1 == __HAL_RCC_GPIOC_IS_CLK_ENABLED(), "Expected GPIOC clock to be enabled in led_rgb_init().");
    TEST_ASSERT_MESSAGE(1 == __HAL_RCC_TIM3_IS_CLK_ENABLED(), "Expected TIM3 clock to be enabled in led_rgb_init().");
}
...
TEST(RGB_LED, TimerConfig)
{
    ...
    TEST_ASSERT_MESSAGE((mockTIM3.CCMR1 & timerOCInitSpy.OCMode) == timerOCInitSpy.OCMode, "Expected TIM_CHANNEL_1 to be configured using HAL_TIM_PWM_ConfigChannel().");
    TEST_ASSERT_MESSAGE((mockTIM3.CCMR2 & timerOCInitSpy.OCMode) == timerOCInitSpy.OCMode, "Expected TIM_CHANNEL_3 to be configured using HAL_TIM_PWM_ConfigChannel().");
    TEST_ASSERT_MESSAGE((mockTIM3.CCMR2 & (timerOCInitSpy.OCMode << 8U)) == (timerOCInitSpy.OCMode << 8U), "Expected TIM_CHANNEL_4 to be configured using HAL_TIM_PWM_ConfigChannel().");

    /* Check channels 1, 3 and 4 were started using PWM_Start */
    TEST_ASSERT_MESSAGE((mockTIM3.CCER & (uint32_t)(TIM_CCx_ENABLE << TIM_CHANNEL_1)) == (uint32_t)(TIM_CCx_ENABLE << TIM_CHANNEL_1), "Expected TIM_CHANNEL_1 to be started using HAL_TIM_PWM_Start().");
    TEST_ASSERT_MESSAGE((mockTIM3.CCER & (uint32_t)(TIM_CCx_ENABLE << TIM_CHANNEL_3)) == (uint32_t)(TIM_CCx_ENABLE << TIM_CHANNEL_3), "Expected TIM_CHANNEL_3 to be started using HAL_TIM_PWM_Start().");
    TEST_ASSERT_MESSAGE((mockTIM3.CCER & (uint32_t)(TIM_CCx_ENABLE << TIM_CHANNEL_4)) == (uint32_t)(TIM_CCx_ENABLE << TIM_CHANNEL_4), "Expected TIM_CHANNEL_4 to be started using HAL_TIM_PWM_Start().");
...
TEST(RGB_LED, DefaultBrightness)
{
    ...
    TEST_ASSERT_EQUAL_UINT16(0, led_rgb_get_green());
    TEST_ASSERT_EQUAL_UINT16(0, led_rgb_get_blue());
}

TEST(RGB_LED, SetBrightness)
{
    led_rgb_set_red(1234);
    TEST_ASSERT_EQUAL_UINT16_MESSAGE(1234, led_rgb_get_red(),"Expected red to set/get compare register for TIM3 CH1");
    led_rgb_set_green(4321);
    TEST_ASSERT_EQUAL_UINT16_MESSAGE(4321, led_rgb_get_green(),"Expected green to set/get compare register for TIM3 CH3");
    led_rgb_set_blue(1324);
    TEST_ASSERT_EQUAL_UINT16_MESSAGE(1324, led_rgb_get_blue(),"Expected blue to set/get compare register for TIM3 CH4");
}

TEST_GROUP_RUNNER(RGB_LED)
{
    RUN_TEST_CASE(RGB_LED, PinConfig);
    RUN_TEST_CASE(RGB_LED, ClocksInRGBInit);
